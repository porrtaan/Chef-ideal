...
<html class="scroll-smooth overflow-x-hidden" lang="en">
  <head>
    ...
    .pagination-link:hover {
        background-color:rgba(153, 0, 255, 0.63);
        color: white;
        border-color:rgba(153, 0, 255, 0.63);
    }
...
<div class="flex items-center gap-3 py-3 px-6 mb-[22px]"><img src="assets/images/icons/icon-add-square.svg" alt="add icon">
      ...
      <main class="overflow-x-scroll scrollbar-hide flex flex-col justify-between pt-[42px] px-[23px] pb-[28px]">
        <div>
          <div class="flex items-center justify-between mb-[19px]">
            <div>
              <h2 class="capitalize text-gray-1100 font-bold text-[28px] leading-[35px] dark:text-gray-dark-1100 mb-[13px]">Produtos ativados</h2>
              <div class="flex items-center justify-between">
                <div class="flex items-center text-xs text-gray-500 gap-x-[11px]">
                  <div class="flex items-center gap-x-1"><img src="assets/images/icons/icon-home-2.svg" alt="home icon"><span class="capitalize">home</span></div><img src="assets/images/icons/icon-arrow-right.svg" alt="arrow right icon"><span class="capitalize text-color-brands">Produtos ativados</span>
                </div>
              </div>
            </div>
            <div class="flex"><a class="list-grid-btn flex items-center gap-x-2 rounded-l-2xl bg-neutral active px-[17px] py-[14px] dark:bg-dark-neutral-border" href="product-list.php"><img src="assets/images/icons/icon-row-vertical.svg" alt="row vertical icon"><span class="text-xs text-[#8083A3]">Ativados</span></a><a class="list-grid-btn flex items-center gap-x-2 bg-neutral rounded-r-2xl px-[17px] py-[14px] dark:bg-dark-neutral-border" href="desativados_list.php"><img src="assets/images/icons/icon-grid.svg" alt="grid icon"><span class="text-xs text-[#8083A3]">Desativados</span></a></div>
          </div>
          <div class="border p-6 bg-neutral-bg rounded-2xl border-neutral pb-0 overflow-x-scroll scrollbar-hide dark:bg-dark-neutral-bg dark:border-dark-neutral-border mb-[52px] xl:overflow-x-hidden">
            <div class="text-base leading-5 text-gray-1100 font-semibold mb-6 dark:text-gray-dark-1100">Produtos</div>
            <table class="w-full min-w-[900px]">
              <thead>
                <tr class="border-b border-neutral dark:border-dark-neutral-border pb-[15px]"> 
                  ...
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(2)">Preço uni.<span id="arrow-2"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(3)">Stock<span id="arrow-3"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(4)">Categoria<span id="arrow-3"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(5)">Marca<span id="arrow-3"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(7)">Imagem Principal <span id="arrow-4"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(8)">Editar<span id="arrow-5"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-center pb-[15px] dark:text-gray-dark-400" onclick="sortTable(9)">Desativar<span id="arrow-6"><i class="fa-solid fa-arrows-up-down"></i></span></th>
              </thead>
              <tbody> 
              <?php
    include 'connect.php';
    ...
    $sql_total = "SELECT COUNT(*) AS total FROM produto WHERE estado = 'A'";
    $stmt_total = $cn->prepare($sql_total);
    $stmt_total->execute();
    $total_produtos = $stmt_total->get_result()->fetch_assoc()['total'];
    $total_paginas = ceil($total_produtos / $produtos_por_pagina);

    // Buscar produtos com limite e offset para paginação
    $sql_produtos = "SELECT produto.*, marca.nome_marca, GROUP_CONCAT(categoria.nome_categoria SEPARATOR ', ') AS categorias
                    FROM produto
                    LEFT JOIN marca ON produto.id_marca = marca.id_marca
                    LEFT JOIN produto_categoria ON produto.id_prod = produto_categoria.id_prod
                    LEFT JOIN categoria ON produto_categoria.id_categoria = categoria.id_categoria
                    WHERE produto.estado = 'A'
                    GROUP BY produto.id_prod
                    LIMIT ? OFFSET ?";
    $stmt_produtos = $cn->prepare($sql_produtos);
    $stmt_produtos->bind_param('ii', $produtos_por_pagina, $offset);
    $stmt_produtos->execute();
    $resultado_produtos = $stmt_produtos->get_result();

    function formatarEstoque($stock) {
        if ($stock == 0) {
            return '<td style="color: red; font-weight: bold;">' . $stock . '</td>';
        } elseif ($stock >= 10) {
            return '<td style="color: green; font-weight: bold;">' . $stock . '</td>';
        } elseif ($stock < 10 && $stock >= 5) {
            return '<td style="color: #ffc800; font-weight: bold;">' . $stock . '</td>';
        } else {
            return '<td style="color: #ff6f00; font-weight: bold;">' . $stock . '</td>';
        }
    }

    while ($produto = $resultado_produtos->fetch_assoc()) {
        $categorias = $produto['categorias'] ?: 'Sem categoria!';
        echo '<tr class="border-b text-normal text-gray-1100 border-neutral dark:border-dark-neutral-border dark:text-gray-dark-1100">';
        echo '<td class="py-[17px]"><span>' . $produto['id_prod'] . '</span></td>';
        echo '<td><span>' . $produto['nome'] . '</span></td>';
        echo '<td><span>' . $produto['preco_uni'] . '</span></td>';
        echo formatarEstoque($produto['stock']);
        echo '<td><span>' . $categorias . '</span></td>';
        echo '<td><span>' . $produto['nome_marca'] . '</span></td>';
        echo '<td style="text-align: center;"><img src="uploads/' . $produto['imagem_principal'] . '" alt="Imagem de ' . $produto['nome'] . '" style="max-width: 150px; max-height: 150px; object-fit: contain;"></td>';
        echo '<td style="text-align: center;"><a class="edit-produto" href="edit_form.php?id_prod=' . $produto['id_prod'] . '"><i class="fa-solid fa-pen-to-square"></i></a></td>';
        echo '<td style="text-align: center;">
        <a href="update_produto.php?id_prod=' . $produto['id_prod'] . '" 
...
?>
    ...
    <?php if ($pagina_atual > 1): ?>
        <a href="?pagina=<?php echo $pagina_atual - 1; ?>" class="pagination-link">&laquo; Anterior</a>
    <?php endif; ?>
    
    <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
        <a href="?pagina=<?php echo $i; ?>" class="pagination-link <?php echo ($i == $pagina_atual) ? 'active' : ''; ?>"><?php echo $i; ?></a>
    <?php endfor; ?>
    
    <?php if ($pagina_atual < $total_paginas): ?>
        <a href="?pagina=<?php echo $pagina_atual + 1; ?>" class="pagination-link">Próximo &raquo;</a>
    <?php endif; ?>
</div>
      </main>
    </div>
