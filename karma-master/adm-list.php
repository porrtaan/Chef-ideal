      ...
      <main class="overflow-x-scroll scrollbar-hide flex flex-col justify-between pt-[42px] px-[23px] pb-[28px]">
        <div>
          <div class="flex items-center justify-between mb-[19px]">
            <div>
              <h2 class="capitalize text-gray-1100 font-bold text-[28px] leading-[35px] dark:text-gray-dark-1100 mb-[13px]">Todos Adms</h2>
              <div class="flex items-center justify-between">
                <div class="flex items-center text-xs text-gray-500 gap-x-[11px]">
                  <div class="flex items-center gap-x-1"><img src="assets/images/icons/icon-home-2.svg" alt="home icon"><span class="capitalize">home</span></div><img src="assets/images/icons/icon-arrow-right.svg" alt="arrow right icon"><span class="capitalize text-color-brands">Todos adms</span>
                </div>
              </div>
            </div>
            <div class="flex"><a class="list-grid-btn flex items-center gap-x-2 rounded-l-2xl bg-neutral active px-[17px] py-[14px] dark:bg-dark-neutral-border" href="adm-list.php"><img src="assets/images/icons/icon-row-vertical.svg" alt="row vertical icon"><span class="text-xs text-[#8083A3]">Administrador</span></a><a class="list-grid-btn flex items-center gap-x-2 bg-neutral rounded-r-2xl px-[17px] py-[14px] dark:bg-dark-neutral-border" href="clients-list.php"><img src="assets/images/icons/icon-grid.svg" alt="grid icon"><span class="text-xs text-[#8083A3]">Utilizador</span></a></div>
          </div>
          <div class="border p-6 bg-neutral-bg rounded-2xl border-neutral pb-0 overflow-x-scroll scrollbar-hide dark:bg-dark-neutral-bg dark:border-dark-neutral-border mb-[52px] xl:overflow-x-hidden">
            <div class="text-base leading-5 text-gray-1100 font-semibold mb-6 dark:text-gray-dark-1100">Products</div>
            <table class="w-full min-w-[900px]">
              <thead>
                <tr class="border-b border-neutral dark:border-dark-neutral-border pb-[15px]"> 
                  ...
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(2)">Telefone<span id="arrow-2"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(3)">Email<span id="arrow-3"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(4)">Password<span id="arrow-3"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(5)">Nif<span id="arrow-4"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-left pb-[15px] dark:text-gray-dark-400" onclick="sortTable(6)">Editar<span id="arrow-5"><i class="fa-solid fa-arrows-up-down"></i></span></th>
                  <th class="font-normal text-normal text-gray-400 text-center pb-[15px] dark:text-gray-dark-400" onclick="sortTable(7)">Eliminar<span id="arrow-6"><i class="fa-solid fa-arrows-up-down"></i></span></th>
              </thead>
              <tbody> 
              <?php
include 'connect.php';
...
$sql_produtos = "SELECT * FROM Utilizador where tipo = 'a'";
...
while ($produto = mysqli_fetch_array($resultado_produtos)) {
    ...
    echo '<td class="py-[17px]"><span>'.$produto['id_user'].'</span></td>';
    echo '<td><span>'.$produto['nome'].'</span></td>';
    echo '<td><span>'.$produto['telefone'].'</span></td>';
    echo '<td><span>'.$produto['email'].'</span></td>';
    if ($_SESSION["id_utilizador"] == 1){
        echo '<td><span>'.$produto['pass'].'</span></td>';
    } elseif ($produto['id_user'] == $_SESSION["id_utilizador"]){
        echo '<td><span>'.$produto['pass'].'</span></td>';
    } else {
        echo '<td> ** </td>';
    }
    echo '<td><span>'.$produto['nif'].'</span></td>';
    echo '<td><span>';
    if ($_SESSION["id_utilizador"] != 1 && $produto['id_user'] == 1) {
        echo '<i class="fa-solid fa-lock" style="color: red;"></i></td>';
    } else {
        echo '<a class="edit-user" href="editar_perfil.php?id_user=' . $produto['id_user'] . '"><i class="fa-solid fa-pen-to-square"></i></a></td>';

    }
    echo '<td style="text-align: center; vertical-align: middle;">';
    if ($produto['id_user'] == 1) {
        echo '<i class="fa-solid fa-lock" style="color: red;"></i></td>';
    } else {
        echo '<a href="#" class="delete-user" data-id="' . $produto['id_user'] . '"><i class="fa-solid fa-trash"></i></a></td>';
